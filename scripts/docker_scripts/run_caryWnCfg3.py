...
base_cli_file = "/home/sathwik/Latest/scripts/base_cli.cfg"
...
def main():
    ...
    #for demIdx in range(1,2,1):
        ...
        for simTypeIdx in range(len(simTypes)):
            ...
            current_simType_folder = current_dem_foldername+"/"+current_simType
            if not os.path.exists(current_simType_folder):
            ...
            for vegIdx in range(len(vegetationTypes)):
                ...
                if not os.path.exists(current_veg_directory):
                ...
                for windDirIdx in range(nWindDirs):
                    ...
                    if not os.path.exists(current_wind_directory):
                    ...
                    if current_simType == "momentum":
                        ...
                        filedata = filedata.replace("num_threads                = 8","num_threads                = 1")
                        #filedata = filedata.replace("write_vtk_output           = true","#write_vtk_output           = true") 
                    ...
                    if current_simType == "momentum":
                        ...
                        #print(current_foam_output_directory)
                        #print([ os.path.join(current_dem_foldername,d) for d in current_foam_output_directory ][0])
                        current_foam_output_dir = [ os.path.join(current_dem_foldername,d) for d in current_foam_output_directory ][0]
                        shutil.rmtree( current_foam_output_dir )
                        endTime = time.perf_counter()
